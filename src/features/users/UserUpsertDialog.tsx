    ...
    <Dialog open={open} onClose={onClose} fullWidth maxWidth="sm">
      ...
      <DialogContent>
        <Stack spacing={2} mt={1}>
          <TextField label="Username" value={form.username} onChange={handleChange('username')} size="small" required error={!!errors.username} helperText={errors.username}
            inputProps={{ autoComplete: 'off' }} />
          <TextField label="Email" value={form.email} onChange={handleChange('email')} size="small" error={!!errors.email} helperText={errors.email}
            inputProps={{ autoComplete: 'off' }} />
          <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2}>
            <TextField label="First Name" value={form.first_name} onChange={handleChange('first_name')} size="small" fullWidth />
            <TextField label="Last Name" value={form.last_name} onChange={handleChange('last_name')} size="small" fullWidth />
          </Stack>
          <TextField label="Tenant Id" value={form.tenant_id} onChange={handleChange('tenant_id')} size="small" />
        </Stack>
      </DialogContent>
      <DialogActions>
        <Button onClick={onClose} disabled={submitting}>Cancel</Button>
        <Button onClick={handleSubmit} variant="contained" disabled={submitting}>{isEdit ? 'Save' : 'Create'}</Button>
      </DialogActions>
    </Dialog>
  );
};
