...
import { Box, Paper, Typography, FormControl, InputLabel, Select, MenuItem, Chip, Stack, Button, Divider } from '@mui/material';
import { apiClient } from '@/lib/apiClient';
import { Application, GlobalUser, Group, Role, Permission, ApplicationUser, UserGroup, GroupRole, RolePermission } from '@/lib/types';
...
export const AuthGraphForm: React.FC = () => {
  ...
  return (
    <Paper sx={{ p: 3 }}>
      ...
      <Typography variant="body2" color="text.secondary" mb={3}>
        Configure relationships in a single linear flow (User → Groups → Roles → Permissions, plus Application membership & app roles).
      </Typography>
      <Stack spacing={3}>
        <Box>
          ...
          <FormControl fullWidth size="small">
            ...
            <Select labelId="user-select" label="User" value={selectedUser} onChange={e => setSelectedUser(e.target.value)}>
        ...
        <Box>
          ...
          <Stack direction={{ xs: 'column', md: 'row' }} spacing={2}>
            <FormControl fullWidth size="small" disabled={!selectedUser}>
              ...
              <Select labelId="app-select" label="Application" value={selectedApplication} onChange={e => setSelectedApplication(e.target.value)}>
                {apps.map(a => <MenuItem key={a.application_id} value={a.application_id}>{a.name}</MenuItem>)}
              </Select>
            </FormControl>
            <FormControl fullWidth size="small" disabled={!selectedApplication || !selectedUser}>
              ...
              >
                {appScopedRoles.map(r => <MenuItem key={r.role_id} value={r.role_id}>{r.name}</MenuItem>)}
              </Select>
            </FormControl>
          </Stack>
          <Box mt={1}>
            <Button size="small" variant="contained" disabled={!selectedApplication || !selectedUser || saving} onClick={saveApplicationRoles}>Save App Roles</Button>
          </Box>
        </Box>
        <Divider />
        <Box>
          ...
          <Stack direction="row" flexWrap="wrap" gap={1}>
            ...
            {(!selectedGroupRoleIds.length && !selectedAppRoleIds.length) && <Typography variant="caption" color="text.secondary">No roles yet</Typography>}
          </Stack>
        </Box>
        <Divider />
        <Box>
          ...
          <Stack direction="row" flexWrap="wrap" gap={1}>
            ...
            {!selectedRolePermissionIds.length && <Typography variant="caption" color="text.secondary">No permissions</Typography>}
          </Stack>
        </Box>
      </Stack>
      {(loading || saving) && <Typography mt={3} variant="caption" color="text.secondary">{loading ? 'Loading graph...' : 'Saving...'}</Typography>}
    </Paper>
  );
};
