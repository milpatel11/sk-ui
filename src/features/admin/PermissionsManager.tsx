...
export const PermissionsManager: React.FC<{ title?: string; hideTitle?: boolean; readOnly?: boolean }> = ({title = 'Permissions Manager', hideTitle = false, readOnly = false}) => {
        ...
        <Box>
            ...
            {readOnly && (
                <Alert severity="info" sx={{mb: 2}}>
                    Permissions are managed by system administrators. Tenants cannot create or modify permissions.
                </Alert>
            )}
            {!readOnly && (
                <Stack direction={{xs: 'column', sm: 'row'}} spacing={2} alignItems="flex-end" mb={2}>
                    <TextField label="Permission" fullWidth size="small" value={name}
                               onChange={e => setName(e.target.value)} required/>
                    <TextField label="Description" fullWidth size="small" value={description}
