...
export const LoginForm: React.FC = () => {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const resp = await apiClient.raw.post('http://localhost:8080/api/auth/login', payload);
      // Prefer Authorization header token (backend appends JWT there)
      const authHeader = resp.headers?.authorization || (resp.headers as any)?.Authorization;
      let headerToken: string | undefined = undefined;
      if (authHeader) {
        headerToken = authHeader.startsWith('Bearer ') ? authHeader.substring(7) : authHeader;
      }
      ...
      if (err && err.status === 400) {
        ...
        if (source) {
          ...
          Object.entries(source as Record<string,unknown>).forEach(([k,v]) => { if (typeof v === 'string') fe[k] = v; });
      ...
      {stage === 'credentials' && (
        <form onSubmit={submit}>
          <Stack spacing={2}>
            ...
            <TextField label="Username or Email" value={username} onChange={e => setUsername(e.target.value)} fullWidth size="small" error={!!(fieldErrors.username || fieldErrors.email)} helperText={fieldErrors.username || fieldErrors.email || ''} />
            <TextField label="Password" type="password" value={password} onChange={e => setPassword(e.target.value)} fullWidth size="small" error={!!fieldErrors.password} helperText={fieldErrors.password || ''} />
            {error && <div style={{ color: 'red', fontSize: 12 }}>{error}</div>}
            <Button type="submit" variant="contained" disabled={loading}>
              {loading ? 'Logging in...' : 'Continue'}
            </Button>
            <Typography variant="body2" color="text.secondary">No account? <Link component="button" onClick={()=>router.push('/signup')}>Sign Up</Link></Typography>
            <Typography variant="caption" color="text.secondary">Forgot password? <Link component="button" onClick={()=>router.push('/forgot-password')}>Request reset</Link></Typography>
          </Stack>
        </form>
      )}
      {stage === 'tenant' && (
