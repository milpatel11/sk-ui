        ...
        <Paper variant="outlined" sx={{p: 2}}>
            ...
            <Stack direction={{xs: 'column', md: 'row'}} spacing={2} mb={2}>
                <FormControl fullWidth size="small">
                    ...
                    <Select labelId="app-select-label" label="Application" value={selectedApp}
                            onChange={e => setSelectedApp(e.target.value)}>
                        {applications.map(a => <MenuItem key={a.application_id}
                                                         value={a.application_id}>{a.name}</MenuItem>)}
                    </Select>
                </FormControl>
                <FormControl fullWidth size="small" disabled={!selectedApp}>
                    <InputLabel id="user-select-label">User</InputLabel>
                    <Select labelId="user-select-label" label="User" value={selectedUser}
            ...
            <Box display="flex" gap={2}>
                <Button variant="contained" size="small" disabled={!selectedApp || !selectedUser || saving}
                        onClick={handleSave}>Save</Button>
                <Button variant="text" size="small" disabled={!selectedApp || !selectedUser || saving}
