...
import {Box, Card, CardContent, CircularProgress, Fade, IconButton, Stack, Tooltip, Typography} from '@mui/material';
...
const iconForApp = (id: string, name: string) => {
    ...
    if (/finance|account|invoice|statement|ar|ap/i.test(id + name)) return <AccountBalanceIcon fontSize="large"
                                                                                               color="primary"/>;
...
export const ApplicationGrid: React.FC = () => {
    ...
    if (loading) return <Box display="flex" alignItems="center" gap={1}><CircularProgress size={22}/> <Typography
        variant="body2">Loading applications...</Typography></Box>;
    ...
    return (
        <Box maxWidth={1100} mx="auto" pt={isXs ? 1 : 2}>
            <Stack direction={isXs ? 'column' : 'row'} alignItems={isXs ? 'flex-start' : 'center'}
                   justifyContent="space-between" mb={isXs ? 1.5 : 2} px={0.5} gap={isXs ? 1 : 0}>
                <Typography variant={isXs ? 'h6' : 'h5'} fontWeight={600}
                            sx={{letterSpacing: 0.25}}>Applications</Typography>
                {totalPages > 1 && (
                    <Stack direction="row" spacing={1} alignItems="center">
                        <Tooltip title="Previous">
              <span>
                <IconButton size={isXs ? 'medium' : 'small'} disabled={page === 0}
                  ...
                  <ArrowBackIosNewIcon fontSize="inherit"/>
                        ...
                        <Typography variant="caption" sx={{
                            minWidth: isXs ? 80 : 60,
                            textAlign: 'center'
                        }}>Page {page + 1} / {totalPages}</Typography>
                        <Tooltip title="Next">
              <span>
                <IconButton size={isXs ? 'medium' : 'small'} disabled={page >= totalPages - 1}
                            onClick={() => setPage(p => Math.min(totalPages - 1, p + 1))} aria-label="Next page">
            ...
            >
                {currentApps.map(app => {
                    ...
                    return (
                        <Link key={app.application_id} href={target}
                              style={{textDecoration: 'none', display: 'block', height: '100%'}}
                              aria-label={`Open ${ariaLabel}`} title={ariaLabel}>
                                    ...
                                    {showOpenCta && (
                                        <Fade in timeout={200}>
                                            <Typography variant="caption" color="primary" fontWeight={500}
                                                        sx={{fontSize: '0.75rem'}}>Open →</Typography>
                                        </Fade>
                                    )}
                                </CardContent>
                            </Card>
                    ...
                    <Card key={`ph-${i}`} className="app-tile app-placeholder" aria-hidden="true" sx={{minWidth: 0}}>
                        <CardContent sx={{
                            ...
                            <Typography variant="caption" color="text.secondary"
                                        sx={{fontSize: isXs ? '0.65rem' : '0.7rem'}}>Reserved</Typography>
                        </CardContent>
                    </Card>
                ))}
            </Box>

            <Typography variant="caption" color="text.secondary" display="block" mt={isXs ? 1.5 : 2} textAlign="center"
                        sx={{fontSize: isXs ? '0.65rem' : '0.7rem'}}>
