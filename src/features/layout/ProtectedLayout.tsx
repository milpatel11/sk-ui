...
import { AppBar, Toolbar, Box, Button, Typography, Divider, Breadcrumbs, Link as MLink } from '@mui/material';
import NextLink from 'next/link';
import AppsIcon from '@mui/icons-material/Apps';
import { useTenants } from '@/features/tenants/TenantContext';

interface ProtectedLayoutProps { children: React.ReactNode; title?: string; breadcrumbs?: Array<{ label: string; href?: string }>; }
...
export const ProtectedLayout: React.FC<ProtectedLayoutProps> = ({ children, title, breadcrumbs }) => {
  ...
  useEffect(() => {
    ...
    if (isTenantScoped) {
      ...
      if (lockedId) router.replace(`/tenant/${lockedId}`);
    }
  }, [isAuthenticated, ready, routeTenantId, isTenantScoped, router]);
    ...
    <Box display="flex" flexDirection="column" minHeight="100vh">
      <AppBar position="static" color="default" elevation={1}>
        <Toolbar sx={{ gap: 2, minHeight: 56 }}>
          ...
          <Box flexGrow={1} />
          <Button size="small" onClick={() => { logout(); router.push('/login'); }}>Logout</Button>
        </Toolbar>
      </AppBar>
        ...
        {autoCrumbs.length > 0 && (
          <Breadcrumbs aria-label="breadcrumb" sx={{ mb: 2, fontSize: 13 }}>
            {autoCrumbs.map((c, i) => {
              ...
              if (!c.href || isLast) return <Typography key={i} color="text.primary" sx={{ textTransform: 'capitalize' }}>{c.label}</Typography>;
              return <MLink key={i} component={NextLink} underline="hover" color="inherit" href={c.href} sx={{ textTransform: 'capitalize' }}>{c.label}</MLink>;
            })}
          </Breadcrumbs>
        )}
        {children}
