...
import { ChatThread, ChatMessage, GlobalUser } from '@/lib/types';
...
export const ChatProvider: React.FC<{ children: React.ReactNode; currentUserId: string }> = ({ children, currentUserId }) => {
  ...
  const loadThreads = useCallback(async () => {
    ...
    try { const resp = await apiClient.get('/chat/threads'); setThreads(resp.data || []); } finally { setLoadingThreads(false); }
  }, []);

  const loadUsers = useCallback(async () => {
    try { const resp = await apiClient.get('/users'); setUsers(resp.data || []); } catch {}
  }, []);

  const loadMessages = useCallback(async (threadId: string) => {
    setLoadingMessages(true);
    try { const resp = await apiClient.get(`/chat/messages/${threadId}`); setMessages(resp.data || []); } finally { setLoadingMessages(false); }
  }, []);

  useEffect(() => { loadThreads(); loadUsers(); }, [loadThreads, loadUsers]);
  ...
  const startDirect = async (userId: string, currentUserId: string) => {
    ...
    try {
      ...
      if (existing) { setActiveThreadId(existing.thread_id); return existing.thread_id; }
      const resp = await apiClient.post('/chat/threads', { participant_user_ids: [currentUserId, userId], type: 'DIRECT' });
  ...
  return (
    <ChatContext.Provider value={{ threads, activeThreadId, setActiveThreadId, messages, loadingThreads, loadingMessages, sendMessage, startDirect, users, starting }}>
      {children}
