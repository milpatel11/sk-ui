...
import { Paper, Stack, Typography, Button, Box, TextField, Chip, Divider, CircularProgress, List, ListItem, ListItemText, ListItemSecondaryAction, IconButton, Skeleton, Tooltip } from '@mui/material';
...
interface TenantLite { tenant_id: string; tenant_name: string; tenant_description?: string | null; }
...
const TenantOnboardingPanel: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const handle = setTimeout(async () => {
      ...
      try {
        ...
        const normalized: TenantLite[] = list.map((t: any) => ({ tenant_id: t.tenant_id || t.tenantId || t.id, tenant_name: t.tenant_name || t.tenantName || t.name, tenant_description: t.tenant_description || t.description || null })).filter(x => x.tenant_id && x.tenant_name);
  ...
  useEffect(() => { loadInvitations(); loadJoinRequests(); }, [loadInvitations, loadJoinRequests]);
  ...
  const requestJoin = async (tenantId: string) => {
    ...
    } catch {/* ignore */} finally { setJoinReqLoading(false); }
  };
  ...
  return (
    <Stack spacing={3} sx={{ mb:0 }}>
          ...
          <List dense sx={{ mt: 0.5 }}>
            {invitations.map(inv => (
              <ListItem key={inv.invitation_id} divider sx={{ px:0 }}>
                <ListItemText
                  primary={<Box display="flex" alignItems="center" gap={0.75}><Typography variant="body2" fontWeight={600}>{inv.tenant_name}</Typography>{inv.role_names?.map(r => <Chip key={r} size="small" label={r} />)}</Box>}
                  secondary={<Typography variant="caption" color="text.secondary">{inv.status || 'PENDING'}</Typography>}
                />
                <ListItemSecondaryAction>
                  <Stack direction="row" spacing={1}>
                    <Button size="small" color="primary" variant="contained" onClick={() => accept(inv.invitation_id)} startIcon={<CheckIcon fontSize="inherit" />} sx={{ textTransform:'none', fontSize:12, py:0.5 }}>Accept</Button>
                    <Button size="small" color="inherit" variant="outlined" onClick={() => decline(inv.invitation_id)} startIcon={<CloseIcon fontSize="inherit" />} sx={{ textTransform:'none', fontSize:12, py:0.5 }}>Decline</Button>
                  </Stack>
                </ListItemSecondaryAction>
              </ListItem>
            ))}
      ...
      <Paper elevation={0} sx={panelPaperSx}>
        ...
        <Typography variant="caption" color="text.secondary">Search for a tenant and request access if you are not invited yet.</Typography>
        <TextField
          ...
          fullWidth
          size="small"
          InputProps={{ startAdornment: <SearchIcon fontSize="small" style={{ marginRight:4, opacity:0.65 }} /> }}
        />
        {searchLoading && <Box mt={1} display="flex" alignItems="center" gap={1}><CircularProgress size={18} /> <Typography variant="caption">Searching...</Typography></Box>}
        {searchError && <Typography mt={1} color="error" variant="body2">{searchError}</Typography>}
        {!searchLoading && !!searchResults.length && (
          <List dense sx={{ mt:0.5, border:'1px solid', borderColor:'divider', borderRadius:1, maxHeight:300, overflowY:'auto' }}>
            {searchResults.map(t => {
              const requested = hasRequested(t.tenant_id);
              return (
                <ListItem key={t.tenant_id} divider={false} sx={{ px:1, '&:not(:last-of-type)': { borderBottom:'1px solid', borderColor:'divider' } }} secondaryAction={
                  <Button
                    size="small"
                  ...
                  <ListItemText primary={<Typography variant="body2" fontWeight={500}>{t.tenant_name}</Typography>} secondary={<Typography variant="caption" color="text.secondary">{t.tenant_description || ' '}</Typography>} />
                </ListItem>
              );
            })}
          </List>
        ...
        <Typography variant="caption" color="text.secondary">Prefer your own space? Create a tenant on the right.</Typography>
      </Paper>
    </Stack>
  );
};
