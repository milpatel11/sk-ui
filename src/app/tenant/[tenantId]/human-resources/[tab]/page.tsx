...
export default function HumanResourcesTabPage() {
    ...
    const load = useCallback(async () => {
        ...
        try {
            ...
            setError(e?.message || 'Failed to load HR');
        } finally {
            setLoading(false);
        }
    }, []);
    ...
    const saveShift = async () => {
        if (!userId || !shiftForm.start || !shiftForm.end) return;
        setError(null);
        try {
            const res = await apiClient.post('/hr/shifts', {userId, ...shiftForm});
            setShifts(prev => [res.data as ShiftRec, ...prev]);
            setShiftForm({start: '', end: '', role: '', location: ''});
            setShiftDlg(false);
        } catch (e: any) {
    ...
    const saveVacation = async () => {
        if (!userId || !vacForm.startDate || !vacForm.endDate) return;
        setError(null);
        try {
            const res = await apiClient.post('/hr/vacations', {userId, ...vacForm});
            setVacations(prev => [res.data as VacationRec, ...prev]);
            setVacForm({startDate: '', endDate: '', reason: ''});
            setVacDlg(false);
        } catch (e: any) {
            setError(e?.message || 'Failed to request vacation');
        }
    };
    const deleteVacation = async (id: string) => {
        try {
            await apiClient.delete(`/hr/vacations/${id}`);
    ...
    const saveAvailability = async () => {
        if (!userId || !availForm.start || !availForm.end) return;
        setError(null);
        try {
            const res = await apiClient.post('/hr/availability', {userId, ...availForm});
            setAvailability(prev => [res.data as AvailabilityRec, ...prev]);
            setAvailForm({start: '', end: '', notes: ''});
            setAvailDlg(false);
        } catch (e: any) {
    ...
    const deleteAvailability = async (id: string) => {
        try {
            await apiClient.delete(`/hr/availability/${id}`);
    ...
    const attendanceCols: GridColDef[] = [
        {
            field: 'clockInAt', headerName: 'Clock In', width: 190, valueFormatter: (p) => {
                const v = (p as any).value as string | undefined;
                return v ? new Date(v).toLocaleString() : '';
            }
        },
        {
            field: 'clockOutAt', headerName: 'Clock Out', width: 190, valueFormatter: (p) => {
                const v = (p as any).value as string | undefined;
                return v ? new Date(v).toLocaleString() : '';
            }
        },
    ...
    const shiftCols: GridColDef[] = [
        {
            field: 'start', headerName: 'Start', width: 190, valueFormatter: (p) => {
                const v = (p as any).value as string | undefined;
                return v ? new Date(v).toLocaleString() : '';
            }
        },
        {
            field: 'end', headerName: 'End', width: 190, valueFormatter: (p) => {
                const v = (p as any).value as string | undefined;
                return v ? new Date(v).toLocaleString() : '';
            }
        },
        ...
        <Box maxWidth={1400} mx="auto" px={{xs: 1, sm: 2, md: 3}}>
            ...
            <Paper elevation={0} variant="outlined" sx={{p: 1.5, borderRadius: 2}}>
                ...
                <TabPanel active={tab} me="attendance">
                    ...
                    <div style={{width: '100%'}}>
                        <DataGrid rows={attendance} columns={attendanceCols} autoHeight disableRowSelectionOnClick
                ...
                <TabPanel active={tab} me="shifts">
                    <Stack direction="row" justifyContent="space-between" alignItems="center" mb={2}>
                        ...
                        <Button size="small" startIcon={<AddIcon/>} variant="contained"
                                onClick={() => setShiftDlg(true)}>New Shift</Button>
                    </Stack>
                    <div style={{width: '100%'}}>
                        <DataGrid rows={shifts} columns={shiftCols} autoHeight disableRowSelectionOnClick
                                  paginationModel={{pageSize: 10, page: 0}} pageSizeOptions={[10, 20, 50]}
                                  getRowId={(r) => r.id}/>
                    </div>
                </TabPanel>
                ...
                <TabPanel active={tab} me="vacations">
                    <Stack direction="row" justifyContent="space-between" alignItems="center" mb={2}>
                        ...
                        <Button size="small" startIcon={<AddIcon/>} variant="contained" onClick={() => setVacDlg(true)}>Request
                            Vacation</Button>
                    </Stack>
                    <div style={{width: '100%'}}>
                        <DataGrid rows={vacations} columns={vacationCols} autoHeight disableRowSelectionOnClick
                                  paginationModel={{pageSize: 10, page: 0}} pageSizeOptions={[10, 20, 50]}
                                  getRowId={(r) => r.id}/>
                    </div>
                </TabPanel>
                ...
                <TabPanel active={tab} me="availability">
                    <Stack direction="row" justifyContent="space-between" alignItems="center" mb={2}>
                        ...
                        <Button size="small" startIcon={<AddIcon/>} variant="contained"
                                onClick={() => setAvailDlg(true)}>Add Availability</Button>
                    </Stack>
                    <div style={{width: '100%'}}>
                        <DataGrid rows={availability} columns={availabilityCols} autoHeight disableRowSelectionOnClick
                                  paginationModel={{pageSize: 10, page: 0}} pageSizeOptions={[10, 20, 50]}
                                  getRowId={(r) => r.id}/>
                    </div>
                </TabPanel>
            ...
            <Dialog open={shiftDlg} onClose={() => setShiftDlg(false)} fullWidth maxWidth="sm">
                ...
                <DialogContent sx={{display: 'flex', flexDirection: 'column', gap: 2, mt: 1}}>
                    <TextField label="Start" type="datetime-local" value={shiftForm.start}
                               onChange={e => setShiftForm(f => ({...f, start: e.target.value}))} fullWidth/>
                    <TextField label="End" type="datetime-local" value={shiftForm.end}
                               onChange={e => setShiftForm(f => ({...f, end: e.target.value}))} fullWidth/>
                    <TextField label="Role" value={shiftForm.role}
                               onChange={e => setShiftForm(f => ({...f, role: e.target.value}))} fullWidth/>
                    <TextField label="Location" value={shiftForm.location}
                               onChange={e => setShiftForm(f => ({...f, location: e.target.value}))} fullWidth/>
                </DialogContent>
                <DialogActions>
                    <Button onClick={() => setShiftDlg(false)}>Cancel</Button>
                    <Button variant="contained" disabled={!shiftForm.start || !shiftForm.end}
                            onClick={saveShift}>Create</Button>
                </DialogActions>
            </Dialog>
            ...
            <Dialog open={vacDlg} onClose={() => setVacDlg(false)} fullWidth maxWidth="sm">
                ...
                <DialogContent sx={{display: 'flex', flexDirection: 'column', gap: 2, mt: 1}}>
                    <TextField label="From" type="date" value={vacForm.startDate}
                               onChange={e => setVacForm(f => ({...f, startDate: e.target.value}))} fullWidth/>
                    <TextField label="To" type="date" value={vacForm.endDate}
                               onChange={e => setVacForm(f => ({...f, endDate: e.target.value}))} fullWidth/>
                    <TextField label="Reason" value={vacForm.reason}
                               onChange={e => setVacForm(f => ({...f, reason: e.target.value}))} fullWidth/>
                </DialogContent>
                <DialogActions>
                    <Button onClick={() => setVacDlg(false)}>Cancel</Button>
                    <Button variant="contained" disabled={!vacForm.startDate || !vacForm.endDate}
                            onClick={saveVacation}>Submit</Button>
                </DialogActions>
            </Dialog>
            ...
            <Dialog open={availDlg} onClose={() => setAvailDlg(false)} fullWidth maxWidth="sm">
                ...
                <DialogContent sx={{display: 'flex', flexDirection: 'column', gap: 2, mt: 1}}>
                    <TextField label="Start" type="datetime-local" value={availForm.start}
                               onChange={e => setAvailForm(f => ({...f, start: e.target.value}))} fullWidth/>
                    <TextField label="End" type="datetime-local" value={availForm.end}
                               onChange={e => setAvailForm(f => ({...f, end: e.target.value}))} fullWidth/>
                    <TextField label="Notes" value={availForm.notes}
                               onChange={e => setAvailForm(f => ({...f, notes: e.target.value}))} fullWidth/>
                </DialogContent>
                <DialogActions>
                    <Button onClick={() => setAvailDlg(false)}>Cancel</Button>
                    <Button variant="contained" disabled={!availForm.start || !availForm.end}
                            onClick={saveAvailability}>Add</Button>
                </DialogActions>
            </Dialog>
        </Box>
    );
