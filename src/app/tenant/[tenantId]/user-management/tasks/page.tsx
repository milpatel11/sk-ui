...
export default function TasksPage() {
    ...
    return (
        <Box maxWidth={900} mx="auto" py={2}>
            ...
            <Typography variant="body2" color="text.secondary" mb={2}>Tasks aggregated from the Task Management
                service.</Typography>
            {error && <Alert severity="error" sx={{mb: 2}}>{error}</Alert>}
            {loading ? (
                <Box display="flex" alignItems="center" gap={1}><CircularProgress size={18}/><Typography
                    variant="caption" color="text.secondary">Loading tasks…</Typography></Box>
            ) : (
                <Stack spacing={2}>
                    {tasks.map(t => (
                        <Paper key={t.id} sx={{p: 2, display: 'flex', alignItems: 'center', gap: 2}}>
                            ...
                            <Box sx={{flex: 1}}>
                                ...
                                {t.priority && <Typography variant="caption"
                                                           color="text.secondary">Priority: {t.priority}</Typography>}
                            </Box>
