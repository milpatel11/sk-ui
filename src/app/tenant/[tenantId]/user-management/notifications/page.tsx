...
export default function NotificationsPage() {
  return (
    <Box maxWidth={900} mx="auto" py={2}>
      ...
      <Typography variant="body2" color="text.secondary" mb={2}>Central inbox of system & application notifications (placeholder).</Typography>
      <Stack spacing={2}>
        {notifications.map(n => (
          <Paper key={n.id} sx={{ p:2, display:'flex', alignItems:'center', gap:2 }}>
            <Chip label={n.type} size="small" color={n.type === 'SECURITY' ? 'error' : n.type === 'TASK' ? 'warning' : 'default'} />
            <Typography variant="body2" flexGrow={1}>{n.message}</Typography>
            <Typography variant="caption" color="text.secondary">#{n.id}</Typography>
          </Paper>
        ))}
        {!notifications.length && <Typography variant="caption" color="text.secondary">No notifications.</Typography>}
      </Stack>
    </Box>
  );
}
