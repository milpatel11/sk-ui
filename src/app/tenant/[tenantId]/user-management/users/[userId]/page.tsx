...
import { Application, Role, Permission, Group, GlobalUser, GroupRole, RolePermission, UserGroup, ApplicationUser } from '@/lib/types';
...
export default function UserDrillDownPage() {
  ...
  <Button component={Link} href="/apps/user-management/users" size="small" variant="outlined" sx={{ mb: 2 }}>Back to Users</Button>
      {!user && (loading ? <Typography variant="body2">Loading...</Typography> : <Typography variant="body2">User not found.</Typography>)}
      {user && (
        <Stack spacing={3}>
          <Paper sx={{ p:2 }}>
          ...
          <Paper sx={{ p:2 }}>
            ...
            <Stack direction="row" flexWrap="wrap" gap={1}>
              ...
              {!userGroupIds.length && <Typography variant="caption" color="text.secondary">No groups</Typography>}
            </Stack>
          </Paper>
          <Paper sx={{ p:2 }}>
            ...
            <Stack direction="row" flexWrap="wrap" gap={1}>
              ...
              {!effectiveRoleIds.length && <Typography variant="caption" color="text.secondary">No roles</Typography>}
            </Stack>
          </Paper>
          <Paper sx={{ p:2 }}>
            <Typography variant="subtitle1" gutterBottom>Permissions</Typography>
            <Stack direction="row" flexWrap="wrap" gap={1}>
              {effectivePermissions.map(p => <Chip key={p.permission_id} label={p.name} size="small" color="primary" variant="outlined" />)}
              {!effectivePermissions.length && <Typography variant="caption" color="text.secondary">No permissions</Typography>}
            </Stack>
          </Paper>
          <Paper sx={{ p:2 }}>
            <Typography variant="subtitle1" gutterBottom>Applications</Typography>
            <Stack direction="row" flexWrap="wrap" gap={1}>
              {accessibleAppEntities.map(app => <Chip key={app.application_id} label={app.name} size="small" />)}
              {!accessibleAppEntities.length && <Typography variant="caption" color="text.secondary">No applications</Typography>}
            </Stack>
          </Paper>
        </Stack>
      )}
