...
export default function CalendarPage() {
            ...
            <Box sx={{overflowX: 'auto'}}>
                <Box display="grid" sx={{
                    ...
                    {['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map(h => (
                        <Box key={h}
                             sx={{p: 0.75, borderRight: '1px solid', borderColor: 'divider', bgcolor: 'grey.50'}}>
                    ...
                    {days.map((d, i) => {
                        ...
                        return (
                            <Box key={d.toISOString()} onClick={() => drillToDay(d)} sx={{
                                cursor: 'pointer',
                                p: 0.5,
                                ...
                                <Typography variant="caption" fontWeight={isToday ? 700 : 400}
                                            color={isToday ? 'primary.main' : 'text.secondary'}>{d.getDate()}</Typography>
                                <Stack spacing={0.25} mt={0.25}>
                                    {dayEvents.slice(0, 3).map(ev => (
                                        <Tooltip key={ev.id} title={ev.title + ' ' + ev.start.toLocaleTimeString([], {
                                            hour: '2-digit',
                                            minute: '2-digit'
                                        })}>
                                            <Chip size="small" label={ev.title} onClick={(e) => {
                                                e.stopPropagation();
                                                setActiveEvent(ev);
                                            }} sx={{
                                                maxWidth: '100%',
    ...
    return (
        <Box py={2}>
            ...
            <Typography variant="body2" color="text.secondary" mb={2}>Week / Day / Month views. Toggle 24h for extended
                hours. Future: drag/drop, availability, meeting creation, attendee management.</Typography>
                ...
                <Stack direction={{xs: 'column', sm: 'row'}} spacing={2} alignItems={{xs: 'flex-start', sm: 'center'}}
                    ...
                    <Stack direction="column" spacing={1} alignItems={{ xs: 'stretch', sm: 'flex-end' }}>
                        ...
                        <Stack direction="row" spacing={1} alignItems="center">
                            ...
                            {view !== 'month' && (
                                <FormControlLabel
                                    control={<Switch checked={fullDay} onChange={e => setFullDay(e.target.checked)} size="small"/>}
                ...
                {view === 'month' ? (
                    ...
                    <Box sx={{overflowX: 'auto'}}>
                        <Box display="grid" sx={{
                            ...
                            border: '1px solid',
                            borderColor: 'divider',
                            minWidth: days.length < 5 ? 600 : 'auto'
                        }}>
                            ...
                            {hours.map(h => (
                                <React.Fragment key={h}>
                                    ...
                                    {days.map(d => {
                                        ...
                                        return (
                                            <Box key={d.toISOString() + h}
                                                 onDoubleClick={() => openCreate({date: d, hour: h})} sx={{
                                                position: 'relative',
                                                borderTop: '1px solid',
                                                borderColor: 'divider',
                                                borderRight: '1px solid',
                                                ...
                                                {cellEvents.map(ev => (
                                                    <Box key={ev.id} onClick={() => setActiveEvent(ev)} sx={{
                                                        position: 'absolute',
                                                        top: 2,
                                                        left: 2,
                                                        right: 2,
            ...
            {/* Optional info line below the calendar */}
            <Typography variant="caption" color="text.secondary" sx={{ mt: 1 }}>
                {loadingEvents ? 'Loading events...' : `${events.length} events in view`}
            </Typography>
            ...
            <Dialog open={createOpen} onClose={() => setCreateOpen(false)} fullWidth maxWidth="sm">
                ...
                <DialogContent sx={{display: 'flex', flexDirection: 'column', gap: 2, mt: 1}}>
                    ...
                    <Stack direction={{xs: 'column', sm: 'row'}} spacing={2}>
                        <TextField label="Color" type="color" size="small" value={draftColor}
                                   onChange={e => setDraftColor(e.target.value)} sx={{width: 120}}
                                   InputLabelProps={{shrink: true}}/>
                    </Stack>
                ...
                <DialogContent>
                    {activeEvent && (
                        <Stack spacing={1} mt={1}>
                            ...
                            {activeEvent.color && <Stack direction="row" spacing={1} alignItems="center"><InputLabel
                                shrink>Color</InputLabel><Box
                                sx={{width: 20, height: 20, borderRadius: '4px', bgcolor: activeEvent.color}}/></Stack>}
                        </Stack>
                    )}
                </DialogContent>
                <DialogActions>
                    <Button onClick={() => {
                        if (activeEvent) drillToDay(activeEvent.start);
                    }}>Go To Day</Button>
