...
export default function ShuttleOpsPage() {
    ...
    const handleCreate = async () => {
        ...
        try {
            setLoading(true);
            const res = await apiClient.post('/itdn/shuttle/bookings', {
                hotelId: tenantId,
                guestName,
                guestEmail,
                pickupStopId,
            ...
            <Stack direction={{xs: 'column', md: 'row'}} spacing={2} alignItems={{xs: 'stretch', md: 'flex-start'}}>
                <Paper variant="outlined" sx={{p: 2, flex: 1}}>
                    ...
                    <Stack direction={{xs: 'column', sm: 'row'}} spacing={2} alignItems="flex-end">
                        <TextField label="Guest Name" value={guestName} onChange={e => setGuestName(e.target.value)}
                                   size="small" fullWidth/>
                        <TextField label="Guest Email" type="email" value={guestEmail}
                                   onChange={e => setGuestEmail(e.target.value)} size="small" fullWidth/>
                    </Stack>
                    <Stack direction={{xs: 'column', sm: 'row'}} spacing={2} alignItems="flex-end" mt={2}>
                        <TextField select label="Pickup Stop" value={pickupStopId}
                                   onChange={e => setPickupStopId(e.target.value)} size="small" fullWidth>
                            {stops.map(s => <MenuItem key={s.id} value={s.id}>{s.name}</MenuItem>)}
                        </TextField>
                        <TextField select label="Dropoff Stop" value={dropoffStopId}
                                   onChange={e => setDropoffStopId(e.target.value)} size="small" fullWidth>
                            {stops.map(s => <MenuItem key={s.id} value={s.id}>{s.name}</MenuItem>)}
                        </TextField>
                    </Stack>
                    <Stack direction={{xs: 'column', sm: 'row'}} spacing={2} alignItems="flex-end" mt={2}>
                        <TextField label="Scheduled At" type="datetime-local" value={scheduledAt}
                                   onChange={e => setScheduledAt(e.target.value)} size="small" sx={{width: 240}}
                                   InputLabelProps={{shrink: true}}/>
                        <TextField label="Passengers" type="number" value={passengers}
                                   onChange={e => setPassengers(Number(e.target.value) || 1)} size="small"
                                   sx={{width: 140}}/>
                        <Box flexGrow={1}/>
                        <Button variant="contained" onClick={handleCreate}
                                disabled={loading || !guestName || !guestEmail || !pickupStopId || !dropoffStopId}>Book
                            Shuttle</Button>
                    </Stack>
                ...
                <Paper variant="outlined" sx={{p: 2, flex: 1}}>
                    ...
                    <Stack direction="row" spacing={1} mt={1} alignItems="center">
                        {vehicles.map(v => (
                            <Tooltip title={`Nudge ${v.name}`} key={v.id}><IconButton size="small"
                                                                                      onClick={() => nudgeVehicle(v)}><MyLocationIcon
                                fontSize="inherit"/></IconButton></Tooltip>
                        ))}
                        <Box flexGrow={1}/>
