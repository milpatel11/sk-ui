...
// Dynamically import the Leaflet map to avoid window access during SSR
const ShuttleLeaflet = dynamic(() => import('@/features/itdn/ShuttleLeaflet'), {
    ssr: false,
    loading: () => (
...
// Inner content component that uses useSearchParams, wrapped by Suspense in the page export
function ShuttleTrackContent() {
        ...
        <Box maxWidth={900} mx="auto" p={2}>
            ...
            <Paper variant="outlined" sx={{p: 2}}>
                <Stack direction={{xs: 'column', sm: 'row'}} spacing={1} alignItems={{xs: 'flex-start', sm: 'center'}}>
                    ...
                    <Chip label={booking.status.replace('_', ' ')} size="small"
                          color={booking.status === 'DROPPED_OFF' ? 'success' : booking.status === 'RUNNING_LATE' ? 'warning' : 'primary'}/>
                    {typeof booking.etaMinutes === 'number' &&
                        <Chip label={`ETA ${booking.etaMinutes}m`} size="small"/>}
                </Stack>
                <Box mt={2}>
                    <ShuttleLeaflet points={points as any} height={320}/>
                </Box>
                <Typography variant="caption" color="text.secondary" display="block" mt={1}>This page updates
...
export default function ShuttleTrackPage() {
    return (
        <Suspense fallback={
            <Box maxWidth={900} mx="auto" p={2}>
                <Paper variant="outlined" sx={{p: 2, height: 400, display: 'flex', alignItems: 'center', justifyContent: 'center'}}>
